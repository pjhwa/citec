...
### NFS 파일시스템 부하 테스트 스크립트 작성 및 설명

안녕하세요. 사용자의 쿼리를 바탕으로, Linux 환경에서 NFS 마운트된 파일시스템에 부하를 주기 위한 Bash 스크립트를 작성하겠습니다. 이 스크립트는 여러 개의 파일(각각 약 300KB 크기)을 동시에 생성하여 NFS 서버의 쓰기 작업에 부하를 가하는 방식으로 설계되었습니다. 이는 active 서버에서 관찰된 간헐적 hang 상태(파일 이동 시 4초 이상 지연)를 재현하기 위한 테스트 목적으로 적합합니다.
