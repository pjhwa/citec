...
| 시간 (2024-08-26) | 주요 이벤트 | 상세 설명 | 영향 및 결과 |
|--------------------|-------------|-----------|--------------|
| 08:31:33 | NTP 서버 변경 | chronyd가 기존 NTP 서버에서 198.19.1.53으로 재선택 (클럭 동기화 서버 전환). slew 모드로 점진적 시간 조정. | 네트워크 지연의 초기 증상으로 의심. 직접적 서비스 중단 없음. |
| 08:31:49 ~ 08:32:14 | DRBD 타임아웃 시작 | db01에서 db02로의 DRBD 동기화 통신 지연 (ko-count: 6 → 2로 감소). "sending time expired" 로그 발생. | Heartbeat 네트워크 단절 시작. 데이터 동기화 중단, 하지만 MariaDB 서비스는 유지되어야 함. |
| 08:32:18 | DRBD 연결 완전 단절 | 42초 응답 지연 후 연결 해제. 재연결 시도 (sender/receiver 스레드 재시작). | 클러스터 불안정. MaxScale 통신까지 순간 단절되어 고객 서비스 영향 (연결 끊어짐 약 27초). |
| 08:32:39 | db02 단절 감지 | db02에서 "error receiving P_DATA, e: -5" 로그 (I/O error 발생, 278KB 데이터 수신 실패). 연결 상태: db01 (Timeout), db02 (NetworkFailure). | db02 쪽 네트워크 입력 문제 확인. 전체 클러스터 지연 21초 추가. |
| 08:32:59 | DRBD 재연결 성공 | 핸드셰이크 완료, 프로토콜 121 협상. | 연결 복구 시작. |
| 08:33:00 ~ 08:34:46 | 데이터 재동기화 | drbd0: 2.6GB, drbd1: 60MB 재동기화. Pacemaker에서 클러스터 상태 변경 감지. | 데이터 일관성 복구. 서비스 정상화. |
| 전체 기간 | 장애 요약 | VMware 기반 MariaDB VM (db01, db02)에서 Heartbeat 및 Service 네트워크 지연 발생. vCenter/ESXi 이벤트 없음, 스토리지 통계 정상. | 고객 통신 단절 (08:31:51 ~ 08:32:18, 27초). 근본 원인: 네트워크 플랩(flap, 순간 불안정) 추정. 재발 방지: 네트워크 redundancy 강화 추천. |
