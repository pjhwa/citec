...
- **상호 작용**: TSO와 GRO는 독립적이지만, TCP 연결에서 상호 영향을 줍니다. 송신 측 TSO가 큰 데이터를 보내도, 수신 측 GRO가 패킷을 합치면 TCP 스택이 수신 패킷 크기를 기반으로 MSS (Maximum Segment Size)나 TCP window 크기를 계산합니다. 여기서 문제가 발생: <u>GRO로 합쳐진 패킷 크기를 기반으로 MSS 추정이 잘못되면 TCP window가 비정상적으로 작아집니다 (window clamping). 결과적으로 송신 측이 작은 window를 광고받아 작은 패킷만 보내게 되고, TSO가 적용될 여지가 없어집니다.</u>
...
### 5. 참조 자료
- Red Hat 공식 솔루션: NICs with RX acceleration (GRO/LRO) may suffer from bad TCP performance. GRO로 인한 MSS miscalculation 설명. 
