...
- **원인**:
  ...
  - Kubernetes 1.29 이상에서는 admin.conf가 "kubeadm:cluster-admins" 그룹을 사용하며, 이는 RBAC를 통해 관리됨. 반면, super-admin.conf는 "system:masters" 그룹을 포함하여 RBAC를 우회하는 비상용 관리자 권한을 제공. 본 문서의 설치에서 사용자의 경우, admin.conf를 사용 중이며, 이는 RBAC 바인딩에 의존하므로, 바인딩이 누락되면 "Forbidden" 오류가 발생. super-admin.conf는 kubeadm init 중 생성되며, 이는 "system:masters" 그룹을 포함하므로 관리자 권한을 자동으로 부여받음.
  ```bash
...
- **해결 방법**:
  1. **임시 RBAC 비활성화**:
     kube-apiserver.yaml 파일에 `--authorization-mode=AlwaysAllow` 추가한 후 저장. kube-apiserver가 RBAC 비활성화된 상태로 재시작.
     ```bash
