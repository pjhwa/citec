...
#### **1.3 파드 스케줄링 비활성화**
재부팅 중 파드가 다른 노드로 이동하지 않도록 노드를 `unschedulable`로 설정합니다.
- 모든 노드에 대해:
  ```bash
  ...
  node/k4 cordoned
  ```
  - 예: `kubectl cordon k1`, `kubectl cordon k2`, ...
  - 확인:
    ```bash
    citec@k1:~/osh$ kubectl get nodes
    NAME   STATUS                     ROLES           AGE   VERSION
    ...
    ```
    - `SchedulingDisabled` 상태로 표시되어야 합니다.

#### **1.4 중요한 파드 드레인**
노드에서 실행 중인 파드를 안전하게 제거합니다.
- 각 노드에 대해:
  ```bash
  citec@k1:~/osh$ kubectl drain k1 --ignore-daemonsets --delete-emptydir-data --force
  node/k1 already cordoned
  Warning: ignoring DaemonSet-managed Pods: kube-system/calico-node-pxj8c, kube-system/kube-proxy-qjh87, metallb-system/metallb-speaker-wkgnk
  node/k1 drained
  citec@k1:~/osh$ kubectl drain k2 --ignore-daemonsets --delete-emptydir-data --force
  node/k2 already cordoned
  Warning: ignoring DaemonSet-managed Pods: kube-system/calico-node-strt8, kube-system/kube-proxy-hvhp8, metallb-system/metallb-speaker-59qk9
  evicting pod kube-system/coredns-5c55d7d65b-wv9xl
  pod/coredns-5c55d7d65b-wv9xl evicted
  node/k2 drained
  citec@k1:~/osh$ kubectl drain k3 --ignore-daemonsets --delete-emptydir-data --force
  node/k3 already cordoned
  Warning: ignoring DaemonSet-managed Pods: kube-system/calico-node-zlrv7, kube-system/kube-proxy-vx5ns, metallb-system/metallb-speaker-mjt4s
  evicting pod kube-system/coredns-5c55d7d65b-nqqr2
  pod/coredns-5c55d7d65b-nqqr2 evicted
  node/k3 drained
  citec@k1:~/osh$ kubectl drain k4 --ignore-daemonsets --delete-emptydir-data --force
  node/k4 already cordoned
  Warning: ignoring DaemonSet-managed Pods: kube-system/calico-node-jdlpn, kube-system/kube-proxy-g42tp, metallb-system/metallb-speaker-k6q2l
  evicting pod metallb-system/metallb-controller-5f9bb77dcd-tgg9c
  evicting pod kube-system/calico-kube-controllers-6b78c44475-5bt4m
  pod/metallb-controller-5f9bb77dcd-tgg9c evicted
  pod/calico-kube-controllers-6b78c44475-5bt4m evicted
  node/k4 drained
  ```
  - `ignore-daemonsets`: DaemonSet 파드는 드레인되지 않도록 설정.
  - `delete-emptydir-data`: EmptyDir 볼륨 데이터 삭제 허용.
  - `force`: 비관리 파드도 강제로 제거.
- **참고**: HA 클러스터라면 마스터 노드의 컨트롤 플레인 파드(`kube-apiserver`, `kube-scheduler`, `kube-controller-manager`)가 다른 마스터 노드로 이동합니다. 단일 마스터 노드라면 이 단계에서 클러스터가 일시적으로 중단될 수 있습니다.

---

### **2. 노드 재부팅**
노드를 순차적으로 재부팅하여 클러스터의 안정성을 유지합니다. HA 클러스터라면 마스터 노드를 먼저 재부팅하고, 워커 노드를 나중에 재부팅합니다. 단일 마스터 클러스터라면 마스터 노드 재부팅 시 클러스터가 잠시 중단되므로, 워커 노드들을 순차적으로 재부팅한 후에 마스터 노드를 재부팅합니다.

#### **2.1 워커 노드 재부팅**
1. **워커 노드 순차적 재부팅**:
   - 각 워커 노드에 대해 재부팅 명령을 수행 (예시에서는 k4 노드부터 재부팅 수행):
     ```bash
   ...
   - 재부팅 후 노드 상태 확인:
     ...
     citec@k1:~/osh$ kubectl get nodes
     NAME   STATUS                        ROLES           AGE   VERSION
     k1     Ready,SchedulingDisabled      control-plane   17m   v1.29.15
     k2     Ready,SchedulingDisabled      <none>          16m   v1.29.15
     k3     Ready,SchedulingDisabled      <none>          16m   v1.29.15
     k4     NotReady,SchedulingDisabled   <none>          16m   v1.29.15  # <-- reboot
     ```
...
2. **노드 스케줄링 활성화**:
   ...
   - 확인(아래와 같이 k4 노드가 Ready 상태로 돌아온 후 다음 노드를 재부팅 수행):
     ...
     citec@k1:~/osh$ kubectl get nodes
     NAME   STATUS                     ROLES           AGE   VERSION
     k1     Ready,SchedulingDisabled   control-plane   18m   v1.29.15
     k2     Ready,SchedulingDisabled   <none>          18m   v1.29.15
     k3     Ready,SchedulingDisabled   <none>          18m   v1.29.15
     k4     Ready                      <none>          18m   v1.29.15
     ```
...
3. **파드 상태 점검**:
   ...
   citec@k1:~/osh$ kubectl get nodes
   NAME   STATUS                     ROLES           AGE   VERSION
   k1     Ready,SchedulingDisabled   control-plane   27m   v1.29.15
   k2     Ready                      <none>          26m   v1.29.15
   k3     Ready                      <none>          26m   v1.29.15
   k4     Ready                      <none>          26m   v1.29.15
   citec@k1:~/osh$ kubectl get pods -A -o wide
   NAMESPACE        NAME                                       READY   STATUS    RESTARTS        AGE   IP               NODE   NOMINATED NODE   READINESS GATES
...
#### **2.2 마스터 노드 재부팅**
마스터 노드는 컨트롤 플레인 구성 요소를 실행하므로 더 신중히 재부팅해야 합니다.

1. **HA 클러스터 (다중 마스터)**:
   - 마스터 노드를 하나씩 재부팅:
     ```bash
     ssh <MASTER_NODE> sudo reboot
     ```
   - 재부팅 후 노드 상태 확인:
     ```bash
     kubectl get nodes
   ...
   - 컨트롤 플레인 파드 상태 확인:
     ```bash
     kubectl -n kube-system get pods -l component=kube-apiserver,component=kube-scheduler,component=kube-controller-manager
     ```
   - 다음 마스터 노드로 진행.
