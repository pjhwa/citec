...
# 변경 이력 (최신부터):
#
# Version: 5.0.10 (2026-05-11)
#   [BUG-N P2] compute 프로파일: DEFAULT_SAFE_REMOVE GUI 섹션에 GNOME 관련 관리 도구 누락
#              - 증상: gnome-shell 제거 후 update-manager/update-notifier 등이 KI-06 패키지
#                (notification-daemon, policykit-1-gnome)를 의존성으로 잡아 dpkg --purge 실패
#              - 원인: 패키지명이 gnome-* 패턴과 불일치 — 명시 등록 필요
#              - 수정: DEFAULT_SAFE_REMOVE 에 update-manager, update-notifier,
#                ubuntu-release-upgrader-gtk, network-manager-gnome 추가
#              - 효과: compute 프로파일 execute 시 이 패키지들이 batch purge 대상에 포함되어
#                gnome-shell과 함께 제거 → KI-06 자동 설치 패키지도 의존성 해소 후 정상 제거
#
...
SCRIPT_VERSION="5.0.10"
SCRIPT_DATE="2026-05-11"
...
apt_exec() {
    ...
    if $VERBOSE; then
        ...
        # [BUG-M] prerm hook 등 외부 명령이 주입하는 ANSI 코드를 plain log에서 제거
        local _ansi_esc; _ansi_esc=$(printf '\x1b')
        sed -i "s/${_ansi_esc}\[[0-9;]*[mK]//g" "$PLAIN_LOG_FILE" 2>/dev/null || true
...
DEFAULT_SAFE_REMOVE=(
    ...
    # GNOME 관련 GUI 관리 도구 (gnome-* 패턴 미포함 — 명시 등록)
...
get_direct_dependents() {
    local pkg="$1"
    # [BUG-D] timeout 적용 / [BUG-K] --no-recommends: Recommends를 hard dep으로 오인 방지
    # [BUG-K] || true: rdep 없을 때 grep exit 1 → pipefail 연쇄 방지
    timeout "$APT_CACHE_TIMEOUT" apt-cache rdepends --no-recommends --no-suggests --no-conflicts \
        --no-breaks --no-replaces --no-enhances --installed "$pkg" 2>/dev/null \
        | tail -n +3 | sed 's/^[[:space:]]*//' | grep -v '^$' \
        | grep -vFx "$pkg" | sort -u || true
}
