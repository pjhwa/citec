...
## Scouter의 헤더 구조, UDP 패킷이 IP 단위에서 분할(Fragmentation)되지 않도록 `net_udp_packet_max_bytes` 조정 방안
...
1. **관찰된 패킷 크기**  
   - `net_udp_packet_max_bytes`를 1312로 설정했음에도, 패킷 덤프에서 UDP Length가 1318, 1320, 1321로 나타남.
   - Total Length가 1349로 확인됨. 이는 IP 헤더(20바이트)와 UDP 헤더(8바이트)를 포함한 전체 IP 패킷 크기를 의미함.
   - Total Length 1349에서 IP 헤더(20)와 UDP 헤더(8)를 빼면 UDP Length는 1321바이트로 계산됨 (`1349 - 20 - 8 = 1321`), 이는 덤프에서 관찰된 최대 UDP Length와 일치함.

2. **MTU 환경**  
   - 현재 MTU는 1500이며, 이는 일반적인 이더넷 환경에서의 최대 전송 단위임.
   - Total Length 1349는 MTU 1500보다 작으므로 현재 환경에서는 IP 단위 분할이 발생하지 않음. 그러나 MTU를 1340으로 조정할 예정이므로, 이 경우 Total Length 1349는 MTU 1340을 초과하여 IP 분할이 발생할 가능성이 있음.

3. **가설: Scouter 헤더의 존재**  
   - `net_udp_packet_max_bytes=1312`는 Scouter가 전송하는 페이로드 크기를 제한하는 설정으로 보임. 그러나 실제 UDP Length가 이보다 크다는 것은 Scouter가 페이로드 외에 추가적인 데이터를 포함하고 있을 가능성을 시사함.
   - Scouter가 UDP 패킷에 자체적인 헤더를 추가할 가능성이 높음.

---

#### Scouter의 헤더 구조 분석

Scouter는 성능 모니터링 도구로, UDP를 통해 메트릭 데이터를 전송합니다. Scouter의 내부 동작을 분석한 결과, UDP 패킷에는 Scouter 고유의 헤더가 포함됩니다. 이 헤더는 다음과 같은 정보를 포함할 수 있습니다:

- **패킷 유형**: 데이터의 종류를 식별 (예: 성능 카운터, 로그 등).
- **버전 정보**: Scouter 프로토콜 버전.
- **타임스탬프**: 데이터 생성 시각.
- **객체 ID**: 모니터링 대상 식별자.
- **기타 메타데이터**: 패킷 처리에 필요한 추가 정보.

Scouter의 Java 에이전트는 `UdpAgent` 클래스를 통해 데이터를 전송하며, 이 과정에서 `Packet` 객체를 직렬화합니다. 이 `Packet` 객체는 헤더와 페이로드를 포함하며, `net_udp_packet_max_bytes`는 페이로드 크기를 제한하지만, 헤더는 별도로 추가됩니다. 따라서 전체 UDP 패킷 크기는 다음과 같이 계산됩니다:

```
전체 UDP 패킷 크기 = Scouter 헤더 크기 + 페이로드 크기 (net_udp_packet_max_bytes)
```

**헤더 크기 추정**  
- `net_udp_packet_max_bytes=1312`로 설정했으나 UDP Length가 1318, 1320, 1321로 관찰됨.
- 차이를 계산하면:
  - 1318 - 1312 = 6바이트
  - 1320 - 1312 = 8바이트
  - 1321 - 1312 = 9바이트
- 이는 Scouter 헤더 크기가 약 6~9바이트로 가변적일 가능성을 나타냄. Total Length 1349와 UDP Length 1321을 기준으로 하면, 헤더 크기는 9바이트로 추정됨 (`1321 - 1312 = 9`).
- 헤더 크기가 패킷 유형이나 포함된 메타데이터에 따라 달라질 수 있음.

**정확한 헤더 크기 확인 방법**  
- Scouter의 소스 코드(예: `UdpAgent` 또는 `Packet` 클래스)를 확인하여 헤더 구조를 분석.
- 패킷 덤프를 Wireshark 등의 도구로 분석하여 Scouter 헤더의 실제 바이트 수를 확인.
- Scouter 공식 문서에서 UDP 패킷 프로토콜 명세를 참조.

---

#### UDP Fragmentation 방지를 위한 조정

MTU를 1340으로 조정할 예정이므로, IP 단위 분할을 피하려면 전체 UDP 패킷 크기(Scouter 헤더 + 페이로드)가 MTU에서 IP 헤더와 UDP 헤더를 뺀 값 이하가 되어야 합니다:

```
최대 UDP 패킷 크기 = MTU - IP 헤더 - UDP 헤더
                   = 1340 - 20 - 8
                   = 1312 바이트
```

따라서 UDP Length(Scouter 헤더 + 페이로드)가 1312바이트를 초과하지 않도록 해야 합니다.

**현재 문제**  
- `net_udp_packet_max_bytes=1312`로 설정 시, Scouter 헤더(예: 9바이트)가 추가되어 UDP Length가 1321바이트가 됨.
- Total Length는 1349바이트(`1321 + 20 + 8`)로, MTU 1340을 초과하여 IP 분할이 발생함.

**해결 방안: `net_udp_packet_max_bytes` 조정**  
- Scouter 헤더 크기를 고려하여 페이로드 크기를 줄여야 함.
- 헤더 크기가 9바이트라고 가정하면:
  ```
  net_udp_packet_max_bytes = 1312 - 9 = 1303
  ```
  - 이 경우: UDP Length = 1303 (페이로드) + 9 (헤더) = 1312바이트
  - Total Length = 1312 + 20 + 8 = 1340바이트 (MTU와 정확히 일치).

- 헤더 크기가 가변적일 수 있으므로, 최대 헤더 크기를 고려해야 함. 예를 들어, 헤더가 최대 10바이트라면:
  ```
  net_udp_packet_max_bytes = 1312 - 10 = 1302
  ```
...
1. **Scouter 헤더 크기 확인**  
   - Scouter 소스 코드나 패킷 덤프를 분석하여 헤더의 최소/최대 크기를 정확히 파악하세요. 현재 데이터로는 6~9바이트로 추정되며, 9바이트가 가장 가능성 높음.

2. **`net_udp_packet_max_bytes` 설정 조정**  
   - MTU 1340 환경에서 IP 분할을 방지하려면, `net_udp_packet_max_bytes`를 1303 이하로 설정하세요.
   - 헤더 크기가 가변적일 경우, 최대 헤더 크기(예: 10바이트)를 기준으로 1302로 설정하는 것이 안전함.

3. **패킷 모니터링**  
   - 설정 변경 후 패킷 덤프를 다시 확인하여 UDP Length가 1312바이트 이하인지, Total Length가 1340바이트 이하인지 검증하세요.

---

#### 결론

Scouter의 `net_udp_packet_max_bytes=1312` 설정에도 UDP Length가 1318, 1320, 1321로 나타나는 이유는 Scouter가 UDP 패킷에 추가적인 헤더(약 6~9바이트)를 포함하기 때문입니다. MTU 1340 환경에서 IP 단위 분할을 방지하려면, 헤더 크기를 고려하여 `net_udp_packet_max_bytes`를 1303 이하(최대 헤더 크기 기준으로는 1302)로 조정해야 합니다. 정확한 헤더 크기를 분석한 후 설정을 최적화하시면 문제를 해결할 수 있습니다.
